...
import { saveAppStateAsLayout, exportMallLayout, getActiveLayout } from "@/utils/localStorage-utils"
...
export function LayoutExport() {
  ...
  const handleExport = () => {
    ...
    try {
      ...
      const nameToUse = layoutName.trim() || getActiveLayout() || `mall-layout-${Date.now()}`

      // First save the current state as a layout
      const saveSuccess = saveAppStateAsLayout(nameToUse)

      if (!saveSuccess) {
        throw new Error("Failed to save current layout")
      }

      // Then export the layout as a JSON file
      const exportSuccess = exportMallLayout(nameToUse)
