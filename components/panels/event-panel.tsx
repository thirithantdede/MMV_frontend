...
import { EventElement, MapElement } from "@/types"
...
const EventProperties = memo(function EventProperties({
    ...
    event: EventElement
    onPropertyChange: (property: keyof EventElement, value: any) => void
  }) {
    ...
    const handleEventDescriptionChange = useCallback(
      (e: React.ChangeEvent<HTMLTextAreaElement>) => onPropertyChange("description", e.target.value),
      [onPropertyChange],
    )
  
    const handleHostChange = useCallback(
      (e: React.ChangeEvent<HTMLInputElement>) => onPropertyChange("hosts", e.target.value),
      [onPropertyChange],
    )
    ...
    const isCurrentlyActive = useMemo(() => {
      ...
      return now >= startDate && now <= endDate && event.is_active
    }, [event.start_date, event.end_date, event.is_active])
    ...
    return (
      <div className="space-y-4 pt-4">
        <div className="grid gap-2">
          ...
          <Input id="event-start-date" type="date" value={event.start_date || ""} onChange={handleStartDateChange} />
        </div>
  
        <div className="grid gap-2">
          <Label htmlFor="event-end-date">End Date</Label>
          <Input id="event-end-date" type="date" value={event.end_date || ""} onChange={handleEndDateChange} />
        </div>
  
        <div className="grid grid-cols-2 gap-4">
          <div className="grid gap-2">
            <Label htmlFor="event-start-time">Start Time</Label>
            <Input id="event-start-time" type="time" value={event.start_time || ""} onChange={handleStartTimeChange} />
          </div>
          <div className="grid gap-2">
            <Label htmlFor="event-end-time">End Time</Label>
            <Input id="event-end-time" type="time" value={event.end_time || ""} onChange={handleEndTimeChange} />
          </div>
        </div>
  
        <div className="grid gap-2">
          <Label htmlFor="event-host">Host</Label>
          <Input id="event-host" value={event.hosts || ""} onChange={handleHostChange} placeholder="Event Host" />
        </div>
        ...
        <div className="grid gap-2">
          <div className="flex items-center justify-between">
            ...
            <Switch id="event-active" checked={event.is_active || false} onCheckedChange={handleIsActiveChange} />
          </div>
          {isCurrentlyActive && (
        ...
        <div className="grid gap-2">
          <div className="flex items-center justify-between">
            ...
            <Switch id="event-foc" checked={event.is_foc || false} onCheckedChange={handleIsFocChange} />
          </div>
          <p className="text-xs text-muted-foreground">
            {event.is_foc ? "This event is free for all attendees" : "This event requires payment or registration"}
          </p>
        </div>
